...
- New JPG, PNG, and WebP files can be resized to the selected output's cover dimensions and recompressed as WebP before preview, OPFS persistence, video export, and standalone HTML bake.
- The quality slider defaults to 85%. If the generated WebP is not smaller, the original file is retained.
- Each asset reports its original and stored sizes, while the settings panel reports aggregate savings.
- `Optimize Existing Images` applies the current resolution and quality to media already in the project. Optimization is destructive for the stored project copy, so increasing the output resolution later requires re-importing the original.
- Image rows have a dedicated drag handle for quick source ordering. The order is persisted and immediately rebuilds preview and export; focused handles also support the Up/Down arrow keys.

### Encoder

- `index.html` is the normal network-enabled editor and loads pinned encoder packages from their public CDNs only when export needs them.
- `index.local.html` is the offline editor generated by `npm run build:album-local`; it loads the same pinned packages from the generated `album/vendor/` directory.
- Uses Mediabunny + WebCodecs as the primary export path: deterministic canvas frames, H.264/AAC MP4, resolution-aware video bitrate, and offline audio mixing
- When `Background MP4 Encoding` is enabled, a module Worker owns an `OffscreenCanvas`, H.264/AAC encoders, and MP4 muxer. The main renderer transfers one `ImageBitmap` at a time with backpressure, keeping controls responsive without duplicating composition rules.
- Browsers without Worker-side OffscreenCanvas/WebCodecs support automatically use the existing main-thread encoder and then the MediaRecorder fallback if needed.
- Renders as fast as the browser encoder allows instead of waiting for real-time playback
- Preview, seeking, and export share `resolveSourceTime()` and `renderProjectFrame()`, so loop/hold behavior and project-time clamping are consistent across every editor output path
- The preview toolbar can capture the current output frame as PNG or build a compact validation sheet containing the first, middle, final, and video-loop boundary frames
- An active video export exposes `Cancel Export`; cancellation stops the current WebCodecs, MediaRecorder, or ffmpeg path, discards partial output, and releases media resources.
- Keeps `captureStream` + `MediaRecorder` as a compatibility fallback when WebCodecs or an MP4-compatible encoder is unavailable
- The legacy WebM-to-MP4 ffmpeg.wasm conversion remains available only on that fallback path
...
- Imported image, video, and audio bytes are stored as files in OPFS (Origin Private File System) when the page runs in a supported secure browser context. IndexedDB keeps the asset metadata and audio analysis data.
- Existing media blobs previously saved in IndexedDB are migrated to OPFS automatically the next time they are restored. Replacing or deleting an asset also removes its old OPFS file.
- Browsers without OPFS support, and direct `file://` launches, continue to use IndexedDB as a compatibility fallback.
- The editor displays the active storage mode and the browser-reported origin storage usage below the audio duration controls.
...
- `album/vendor/` is intentionally ignored by Git. Run `npm install` and `npm run build:album-local` to regenerate the offline dependencies and `index.local.html`.
- Serve either editor over localhost or HTTPS. Opening module-based editors directly through `file://` is not supported consistently by browsers.
- The export preflight panel checks the selected resolution, frame rate, audio requirement, browser codecs, fallback route, bitrate, and approximate output size before rendering.
...
- Rebuild the ignored offline browser dependencies with `npm install` followed by `npm run build:album-local`. Commit `index.local.html`, but do not commit `album/vendor/` or `node_modules/`.
