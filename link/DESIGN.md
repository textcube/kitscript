...
- **액션 버튼 "더보기" 바텀시트**: 상단 8버튼 그리드 대신 `가져오기`·`＋연락처`·`⋯더보기` 3개만 한 줄로 노출한다(기존 `.top-actions`가 이미 `@media(max-width:1050px)`에서 `grid-template-columns:repeat(3,...)` 3열 그리드였으므로 버튼 3개가 자연스럽게 1행에 들어맞는다). 나머지 6개(`themeBtn`/`demoBtn`/`pasteContactsBtn`/`copyContactsBtn`/`exportBtn`/`resetBtn`)는 DOM에서 지우지 않고 `#moreActionsPanel > #moreActionsSheet` 컨테이너 안으로 마크업만 재배치했다 — id·onclick 핸들러는 그대로이므로 `app.js`의 기존 바인딩을 재사용한다.
